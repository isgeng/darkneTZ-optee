...
void transfer_weights_prime_CA(float *next_layer_W_prime, int next_layer_W_size, int layer_idx) {
    TEEC_Operation op;
    uint32_t origin;
    TEEC_Result res;
    ...
    op.params[0].tmpref.buffer = next_layer_W_prime;
    op.params[0].tmpref.size = sizeof(float) * next_layer_W_size;
    op.params[1].value.a = layer_idx;

    res = TEEC_InvokeCommand(&sess, TRANS_WEIGHTS_PRIME_CMD,
                             &op, &origin);
    if (res != TEEC_SUCCESS)
        errx(1, "TEEC_InvokeCommand(TRANS_WEIGHTS_PRIME_CMD) failed 0x%x origin 0x%x",
             res, origin);
}

void forward_network_correct_CA(float *IR, float *inputX, int IR_size, int input_size, 
                                int layer_idx, int input_is_obfuscated, int net_batch,
                                int next_layer_type)
{
    // 初始化 TA 的参数结构体
    TEEC_Operation op;
    ...
    // 清空操作参数
    memset(&op, 0, sizeof(op));
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INOUT,  // IR
                                    TEEC_MEMREF_TEMP_INPUT,   // inputX
                                    TEEC_MEMREF_TEMP_INOUT,   // 当前层编号、混淆标记、下一层类型
                                    TEEC_NONE);               // 下一层混淆权重
    int passint[3];
    passint[0] = layer_idx;
    passint[1] = input_is_obfuscated;
    passint[2] = next_layer_type;

    op.params[0].tmpref.buffer = IR;
    op.params[0].tmpref.size = sizeof(float) * IR_size * net_batch;
    op.params[1].tmpref.buffer = inputX;
    op.params[1].tmpref.size = sizeof(float) * input_size * net_batch;
    op.params[2].tmpref.buffer = passint;
    op.params[2].tmpref.size = sizeof(passint);

    // 调用 TA 进行校正操作
    res = TEEC_InvokeCommand(&sess, FORWARD_IR_CORRECT_CMD, &op, &origin);
    if (res != TEEC_SUCCESS) {
        errx(1, "TEEC_InvokeCommand(correct) failed 0x%x origin 0x%x", res, origin);
    }
}
...
TEEC_Result delete_secure_object(char *file_name) {
    
    printf("########## delete file %s in ta ###########\n", file_name);
    ...
    res = TEEC_InvokeCommand(&sess, SECURE_STORE_DELETE_FILE_CMD, &op, &origin);
    if (res != TEEC_SUCCESS) {
        printf("Failed to delete file in TA: 0x%x\n", res);
    }
    return res;
}

void secure_store_delete_files(char *file_name) {
    TEEC_Result res;

    res = delete_secure_object(file_name);
    if (res != TEEC_SUCCESS)
        errx(1, "Failed to delete an object");
    printf("===== delete file: %s =====", file_name);
}

void secure_store_read_files(char *file_name) {
    char file_data[1000];
    ...
    op.params[0].tmpref.buffer = file_name;
    op.params[0].tmpref.size = name_len;
    op.params[1].tmpref.buffer = (void *)file_data;
    op.params[1].tmpref.size = sizeof(file_data);

    res = TEEC_InvokeCommand(&sess, SECURE_STORE_READ_FILE_CMD, &op, &origin);
    if (res != TEEC_SUCCESS) {
        printf("Failed to read file in TA: 0x%x\n", res);
    }

    size_t file_size = op.params[1].tmpref.size;
    size_t num_records = file_size / sizeof(BinRecord);  // 计算记录数
    BinRecord *records = (BinRecord *)file_data;
    
    for (size_t i = 0; i < num_records; i++) {
        printf("Key: %d, Value: %f\n", records[i].key, records[i].value);
        printf("Key: %d, Value: %.7f\n", records[i].key, records[i].value);
        printf("Key: %d, Value: %.8f\n", records[i].key, records[i].value);
        unsigned char *raw_data = (unsigned char *)&records[i].value;
        printf("Raw value bytes: %02x %02x %02x %02x \n", 
        ...
        printf("Key: %d, Value: %f\n", records[i].key, float_value);
        printf("Key: %d, Value: %.7f\n", records[i].key, float_value);
    }
}

TEEC_Result write_secure_object(char *file_name, void *file_data, size_t file_size) {
    
    printf("########## write file %s in ta ###########\n", file_name);
    ...
    res = TEEC_InvokeCommand(&sess, SECURE_STORE_WRITE_FILE_CMD, &op, &origin);
    if (res != TEEC_SUCCESS) {
        printf("Failed to write file in TA: 0x%x\n", res);
    }
    return res;
}

void secure_store_wirte_files(char *folder_path) {
    printf("########## read folder path %s ###########\n", folder_path);
    struct dirent *entry;
    ...
    while ((entry = readdir(dp)) != NULL) {
        if (strstr(entry->d_name, ".bin")) {
            ...
            write_secure_object(file_path, file_data, file_size);
            free(file_data);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if(0 == strcmp(argv[1], "secure_storage")){
        printf("################## Begin secure_storage ####################\n");
        char* folder_path = "secure_weights/lenet";
        if(0 == strcmp(argv[2], "write")){
            secure_store_wirte_files(folder_path);
        } else {
            char file_name[50];
            snprintf(file_name, sizeof(file_name), "%s/%s", folder_path, argv[3]);
            if(0 == strcmp(argv[2], "read")){
                secure_store_read_files(file_name);
            } else if(0 == strcmp(argv[2], "delete")){
                secure_store_delete_files(file_name);
            }
        } 
        printf("################## End secure_storage ####################\n");
        return 0;
    }
