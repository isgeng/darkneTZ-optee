...
void *transfer_weights_thread(void *arg) {
    TransferWeightsArgs *args = (TransferWeightsArgs *)arg;

    // 调用 transfer_weights_prime_CA
    transfer_weights_prime_CA(args->weights, args->size, args->layer_idx);
...
void forward_network(network *netp)
{
    ...
    int i;
    // int input_is_obfuscated = 0;  // 标志位：记录当前层输入是否混淆，初始未混淆
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
        // int next_layer_type = 0;
        // float *next_layer_W_prime = NULL;
        // int next_layer_W_size = 0;

        // pthread_t thread_id;
        // int thread_created = 0; // 线程是否被创建的标志

        // if (i < net.n - 1) {
        //     if (net.layers[i + 1].type == MAXPOOL || net.layers[i + 1].type == AVGPOOL) {
        //         next_layer_type = 1;
        //     } else if (net.layers[i + 1].type == CONVOLUTIONAL) {
        //         next_layer_type = 2;
        //     } else if (net.layers[i + 1].type == CONNECTED) {
        //         next_layer_type = 3;
        //     }
        //     if (input_is_obfuscated == 0 && net.layers[i + 1].type == CONVOLUTIONAL) {
        //         next_layer_W_prime = net.layers[i + 1].weights; // 下一层的混淆权重W'
        //         next_layer_W_size = net.layers[i + 1].nweights; // 下一层的混淆权重W'的权重数量
                
        //         // 创建参数结构体
        //         TransferWeightsArgs *args = malloc(sizeof(TransferWeightsArgs));
        //         args->weights = next_layer_W_prime;
        //         args->size = next_layer_W_size;
        //         args->layer_idx = i + 1;

        //         // 创建线程
        //         if (pthread_create(&thread_id, NULL, transfer_weights_thread, args) != 0) {
        //             perror("Failed to create thread");
        //             free(args); // 确保分配的内存被释放
        //             // 处理错误
        //         } else {
        //             thread_created = 1;
        //         }
        ...
        }else // forward in REE
        {
            ...
            //     // TODO: 判断当前层是否需要校正、哪种校正方式（卷积层需要、池化层..不需要？）
            //     // l.output 作为IR传递到TA校正，同时传递输入X
            //     forward_network_correct_CA(l.output, net.input, l.outputs, l.inputs, net.index, 
            //                                 input_is_obfuscated, net.batch, 
            //                                 next_layer_type);
